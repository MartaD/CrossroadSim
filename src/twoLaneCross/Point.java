...
public class Point {
	...
	public void brake(int x, int y, Point[][] points, LinkedList<TrafficLights> lights){
		...
		if(type==1){
			...
			if(nearestLights!=null)
				lightDist = x-nearestLights.getLocation()>=0?x-nearestLights.getLocation():null;
			else
				lightDist = null;	
			...
			if(nearestLights!=null)
				lightDist = y-nearestLights.getY()>=0?y-nearestLights.getY():null;
			else
				lightDist = null;	
		}
		
		//speed modification
		if(lightDist==null || (nearestLights!=null && nearestLights.isGreen())){
			if(dist!=null)
